...
function termToDeBruijnIndexed(term, ctx) {
    if (term instanceof exprs.Var) {
        const n = ctx.findIndexBy(x => x === term.name);
        if (n >= 0) {
            return new ix.Var(n);
        }
        else {
            throw new errors.CompileError([term.pos], "unbound variable: " + term.name);
        }
    }
...
function defsToDeBruijnIndexed(defs, ctx) {
    ...
    for (const def of defs) {
        const term = termToDeBruijnIndexed(def.term, c);
        terms.push(term);
        c = c.cons(def.name);
    }
...
function lambdaLiftWithLevevl(term, toplevel) {
    if (term instanceof ix.Var) {
        ...
        const body = lambdaLiftWithLevevl(term.body, false);
        ...
        const expr = lambdaLiftWithLevevl(term.expr, toplevel);
        if (expr instanceof ix.Let) {
            ...
            const body = lambdaLiftWithLevevl(term.body, toplevel);
            if (toplevel) {
                return new ix.Let(expr, body);
            }
            else {
